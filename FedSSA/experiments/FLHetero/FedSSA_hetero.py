...
from experiments.FLHetero.Models.CNNs import CNN_1,CNN_2,CNN_3,CNN_4,CNN_5
...
def train(data_name: str, num_classes: int, data_path: str, classes_per_node: int, num_nodes: int, frac: float,
    ...
    for i in range(num_nodes):
        ...
        Local_PMs[i] = net_set[i % 5].state_dict()
    ...
    decay_rounds = 2
    miu_0 = 0.5 # 0.1,0.3,0.5,0.7,0.9,1.0
    save_path = Path(save_path)
    save_path.mkdir(parents=True, exist_ok=True)
    with open(str(save_path / f"Hetero_FedSSA_DecayRound{decay_rounds}_miu0_{miu_0}_R{steps}_N{num_nodes}_C{frac}_E{inner_steps}_NOIID_{data_name}_class_{classes_per_node}.csv"),'w', newline='') as file:
                ...
                for key, _ in header.items():
                    for cls in Owned_All_Classes[node_id]:
                        if step <= decay_rounds: # 10
                            alpha = miu_0 * np.cos(step*np.pi/(decay_rounds*2)) # 20
                        else:
                            alpha = 0
                        # logging.info(f'Class:{cls} | Number of clients about cls: {len(cls_collect)} | alpha:{alpha}')
        ...
        # Collect representations-prediction class of each seen sample of each client
        Collections = defaultdict()

        for i in range(num_nodes):
            node_id = i
            Collections[node_id] = []
            testloader = nodes.test_loaders[node_id]
            model = net_set[node_id % 5]
            model_paras = Local_PMs[node_id]
            model.load_state_dict(model_paras)

            for batch in testloader:
                img, label = tuple(t.to(device) for t in batch)
                pred, rep = model(img)
                Collections[node_id].append([rep.cpu().detach(), label.cpu().detach(), [node_id % 5]*len(label), pred.cpu().detach()])

        print('Finished Collection!')
        with open(f"Results/temp/Collection_FedSSA_{data_name}_N{num_nodes}.pkl", "wb") as file:
            pickle.dump(Collections, file)

if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="FedSSA Heterogeneous Experiment"
    )
    ...
    parser.add_argument(
        "--data-name", type=str, default="cifar10", choices=['cifar100', 'cifar100','mnist'], help="dir path for MNIST dataset"
    )
    ...
    parser.add_argument("--frac", type=int, default=1, help="fraction")
